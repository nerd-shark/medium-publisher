...
changes-from-v4: "AI-tell removal — broke aphoristic section openers, rewrote Not-X-Y patterns, varied sentence rhythm, softened dramatic short sentences, roughened a few too-polished closers."
---
...
The first 10 minutes of an incident determine whether it's a 30-minute blip or a 4-hour catastrophe. Most teams burn those minutes trying to figure out who's in charge.
...
**Minutes 0-1: Acknowledge.** Say "I'm looking at this" in the incident channel. That's it. Just break the silence. The worst dynamic during an incident is everyone assuming someone else is handling it. That one message collapses the bystander effect instantly.
...
Engineers have a strong instinct to understand before acting. That instinct is great during design reviews. During incidents, it'll cost you. Mitigate first. Understand later.

"But if we just rollback, we'll have to fix it later."

Yeah, that's the idea. Fix it later when you're awake, caffeinated, and not hemorrhaging revenue.

## The Incident Commander: Coordinator, Not Hero

The IC role gets misunderstood more than any other part of incident response.

The IC doesn't need to be the most senior person in the room or the best debugger. They need to be the best coordinator — someone who can track multiple workstreams, make decisions under pressure, and keep communication flowing without getting pulled into technical rabbit holes.
...
## Communication: Silence Is Worse Than Bad News

During an incident, people fill information vacuums with worst-case assumptions. Regular updates — even "no change, still investigating" — reduce anxiety and keep people from freelancing their own investigations.
...
External status page updates within 5 minutes of a SEV1 declaration. Customers would rather see "We're aware of an issue affecting checkout and are working on it" than nothing. Nothing makes them check Twitter, where the narrative is already out of your control.
...
A good runbook is a step-by-step procedure for a specific failure scenario. Not a vague "investigate and resolve" — that's a wish list, not a runbook. A good runbook tells you exactly what to check, in what order, and what to do based on what you find.
...
**Contributing factors**: I'd avoid the phrase "root cause" here. Incidents rarely have a single root cause — they have a combination of conditions that together produced the failure. The Navy figured this out decades ago: every investigation into a ship collision finds multiple contributing factors — the watch officer misread the radar, AND the helmsman was fatigued, AND the bridge-to-bridge radio was on the wrong channel, AND the shipping lane was poorly charted. No single failure sinks the ship; it's the alignment of all of them at once. Software incidents work the same way. The deployment was bad AND the canary analysis missed it AND the circuit breaker threshold was too high AND the on-call engineer was in a different timezone. All of those contributed. Fixing only one leaves you exposed to the others.

**Action items**: Specific, owned, and deadlined. "Improve monitoring" is not an action item. "Add latency P99 alert for payment service with 5-second threshold, owned by Sarah, due by April 15" is an action item. Track completion. Unfinished action items are worse than having none — they create a false sense of progress while the same gaps sit open.
...
Share the review widely. If the only people who learn from an incident are the ones who were in the war room, you've left most of the value on the table.

## Building the Muscle

Teams that don't practice incident response get slower over time, not faster. The skills atrophy.

**Game days**: Scheduled incident simulations where you inject a realistic failure and run through the full response — coordination, communication, and decision-making, not just the technical fix. Honestly, the technical fix is usually the straightforward part. It's the coordination that falls apart under pressure.
...
Teams that practice regularly handle real incidents measurably faster. The coordination becomes automatic. The runbooks get tested. The severity framework gets internalized. When the real 3 AM call comes, the response is rehearsed, not improvised.
...
You won't get this perfect on the first try — I don't think anyone does. The goal is reducing the gap between "alert fires" and "someone competent is coordinating the response" to under 5 minutes. That single metric drives everything else downstream.
...
**Previous Article**: [The Bulkhead Pattern: Isolating Failure Domains Within Services](https://medium.com/@the-architect-ds/the-bulkhead-pattern-isolating-failure-domains-within-services-f0d6f5a5341e) *(Part 7)*

**Next Article**: [Database Resilience: When Your Data Layer Fails](link) *(Part 9 — Coming soon!)*

---

*This is Part 8 of the Resilience Engineering series. Read [Part 1: Cell-Based Architecture](https://medium.com/@the-architect-ds/building-bulletproof-systems-cell-based-architecture-and-circuit-breakers-explained-15e5d6838d97), [Part 2: Chaos Engineering](https://medium.com/@the-architect-ds/chaos-engineering-breaking-things-on-purpose-so-they-dont-break-by-accident-f7b810eef514), [Part 3: The $10M Blind Spot](https://medium.com/@the-architect-ds/the-10m-blind-spot-why-your-monitoring-is-lying-to-you-3d6aecd73209), [Part 4: When Everything Fails](https://medium.com/gitconnected/when-everything-fails-the-art-of-failing-gracefully-2106d14d6618), [Part 5: The Day AWS Went Down](https://medium.com/gitconnected/the-day-aws-went-down-building-systems-that-survive-infrastructure-apocalypse-208268d094b5), [Part 6: Rate Limiting and Backpressure](https://medium.com/gitconnected/rate-limiting-and-backpressure-protecting-systems-from-themselves-72a3f9162ea8), and [Part 7: The Bulkhead Pattern](https://medium.com/@the-architect-ds/the-bulkhead-pattern-isolating-failure-domains-within-services-f0d6f5a5341e).*
