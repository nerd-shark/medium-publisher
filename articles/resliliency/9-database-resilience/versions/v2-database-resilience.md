...
changes-from-v1: "Expanded all sections with rough prose and detailed technical examples. Added connection pool math, failover timeline details, backup strategy pyramid with concrete RPO/RTO numbers. Fleshed out slow query handling with specific detection patterns. Added Monday morning checklist with day-by-day actions."
---
...
[The math works out roughly like this: 50 connections × 5-second timeout = 250 seconds of total connection-time before pool exhaustion. At 100 requests/second, that's about 25-30 seconds before the queue starts backing up. Faster request rates = faster pool exhaustion. This is why high-traffic systems fail faster during database incidents — counterintuitive but true.]
...
[Should add a section here about pool sizing math — the formula is roughly: pool_size = (peak_concurrent_requests × avg_query_time) / 1000. So if you have 200 concurrent requests and queries average 10ms, you need 200 × 10 / 1000 = 2 connections. In practice you want 3-5x headroom, so 6-10 connections. Most teams set pool size way too high because they confuse "concurrent requests" with "requests per second." 1000 RPS with 10ms queries only needs 10 concurrent connections.]
...
[Need to add: replica health checking. If a replica falls too far behind on replication (lag > 30 seconds), stop routing reads to it. A replica serving 30-second-old data is worse than no replica at all for some use cases. Monitor `pg_stat_replication` on PostgreSQL or `Seconds_Behind_Master` on MySQL.]
...
- 0-10 seconds: RDS detects primary failure (health check interval)
- 10-30 seconds: RDS initiates failover, promotes standby
- 30-60 seconds: DNS propagation (the RDS endpoint resolves to new IP)
- 60-90 seconds: Application connections drain and reconnect
- 90-120 seconds: Full recovery, all connections established
The DNS propagation step is the one that surprises people. Even with low TTLs, some connection pools cache DNS aggressively. Set your connection pool's DNS TTL to match your database's failover time.]
...
[Split-brain is rare with managed services but common with self-managed clusters. If you're running Patroni for PostgreSQL HA, the fencing mechanism is critical. Without it, you will eventually get split-brain, and reconciling divergent writes is one of the hardest problems in distributed systems. Use managed services unless you have a very good reason not to.]
...
[Consider separate circuit breakers for reads vs writes. A write failure (primary down) shouldn't block reads (replicas still up). And a read failure on one replica shouldn't block reads on other replicas. Granular circuit breakers give you more precise degradation.]
...
[Real numbers from a 200GB PostgreSQL database on RDS:
- Snapshot creation: ~15 minutes (no downtime, uses EBS snapshots)
- Snapshot restore to new instance: ~25 minutes
- Point-in-time recovery: ~30-45 minutes (depends on transaction log volume)
- Cross-region snapshot copy: ~20 minutes
- Full restore from cross-region backup: ~45-60 minutes
These numbers scale roughly linearly with database size. A 1TB database takes about 5x longer.]
...
[Connection storm mitigation: add jitter to reconnection logic. Instead of all 10 instances reconnecting at t=0, have each wait a random 0-5 seconds. This spreads the connection storm over 5 seconds instead of hitting the database with 500 simultaneous connection requests. Most connection pools support this natively — look for "connection backoff" or "reconnect jitter" settings.]
