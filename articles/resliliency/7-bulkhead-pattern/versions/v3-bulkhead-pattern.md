...
subtitle: "One slow API call just exhausted your entire thread pool. 200 healthy endpoints are now returning 503 because of one bad dependency."
...
Part 7 of my series on Resilience Engineering. Last time, we explored [rate limiting and backpressure](link) — teaching systems to say "no" before they drown. This time: what happens when the damage is already inside the walls, and one bad dependency takes down everything else with it. Follow along for more deep dives into building systems that don't fall apart.

## The Thread Pool That Ate Everything

Tuesday afternoon, about 2:30. An e-commerce platform I was consulting for handles 50,000 requests per minute across maybe 200 endpoints. Everything looks fine. Dashboards are green. Then a third-party recommendation service — one of those "customers who bought X also bought Y" APIs — starts responding slowly. Not failing. Just slow. 30 seconds instead of 200 milliseconds.
...
And then checkout goes down. Not because anything is wrong with checkout. Not because the payment gateway is slow. Not because the database is overloaded. Checkout goes down because there are literally no threads left to serve it. Product search goes down. User profiles go down. The health check endpoint goes down, which makes the load balancer think the whole server is dead, which makes things even worse.
...
The bulkhead pattern is simple in concept: give each critical dependency or functionality its own isolated pool of resources. When one pool is exhausted, the others continue operating normally.

Instead of one shared thread pool of 200 threads serving everything, you create separate pools. The checkout flow gets 80 threads — it's your revenue. Product search gets 60 threads. Recommendations get 30 threads. Everything else shares the remaining 30.
...
The key insight: when the recommendations pool is exhausted, `handle_checkout` is completely unaffected. The failure is contained within its bulkhead.

## What to Isolate

Not everything needs its own bulkhead. Over-isolation wastes resources — 10 pools of 20 threads each is less efficient than one pool of 200 threads when everything is healthy. The goal is to isolate the things most likely to fail or most critical to protect.

Isolate by external dependency. Every third-party service call should have its own resource pool. Third-party services are the most common source of latency spikes and failures — you don't control their SLAs, their deployments, or their capacity planning. The recommendation service, the payment gateway, the shipping calculator, the fraud detection API — each gets its own bulkhead.

Isolate by criticality. Your revenue-generating endpoints (checkout, payments) should never compete for resources with non-critical endpoints (analytics, A/B tests, personalization). Give critical paths dedicated resources that can't be consumed by anything else.
...
A practical starting point: one bulkhead per external dependency, one bulkhead for your critical path, and one shared bulkhead for everything else. That's typically 5-8 bulkheads for a medium-complexity service. You can refine from there based on production data.

## Beyond Thread Pools: Connection Pools, Memory, and CPU

Thread pool isolation is the most common form of bulkheading, but it's not the only one. Every shared resource is a potential failure propagation path.

Connection pools are the second most common culprit. Your service has a database connection pool of 50 connections shared across all endpoints. A slow query from the reporting endpoint holds connections for 30 seconds. The connection pool fills up. The checkout endpoint can't get a database connection. Revenue stops.

The fix: separate connection pools for critical and non-critical queries. Or better yet, separate database read replicas — the reporting endpoint queries the replica, the checkout endpoint queries the primary. They can't interfere with each other because they're hitting different database instances entirely.
...
CPU isolation is handled well by container orchestration. Kubernetes CPU limits and requests ensure one pod can't starve others on the same node. But within a single service, CPU-intensive operations (image processing, encryption, compression) should be offloaded to dedicated worker pools so they don't block request-handling threads.

## Bulkheads and Circuit Breakers: Better Together

Bulkheads and circuit breakers solve different problems and work best in combination.
...
Without a circuit breaker, a slow dependency fills its bulkhead with blocked threads. The bulkhead prevents cascade failure, but those threads are wasted — sitting idle, waiting for timeouts. With a circuit breaker, the circuit opens after detecting the slowness, and requests fail fast instead of blocking. The bulkhead threads are freed immediately.

Without a bulkhead, a circuit breaker protects against a failed dependency but doesn't prevent resource exhaustion during the detection window. It takes time (typically 5-10 failed requests) for the circuit breaker to open. During that window, threads are being consumed. With a bulkhead, even during the detection window, only the isolated pool is affected.

The combination is powerful: the bulkhead contains the blast radius while the circuit breaker minimizes the duration. Together, a slow dependency consumes a limited number of threads (bulkhead) for a limited time (circuit breaker) before requests fail fast with a fallback response.
...
Sizing bulkheads wrong is worse than not having them. Too small, and you're artificially limiting throughput during normal operation. Too large, and the bulkhead doesn't actually protect anything.
...
This is why short timeouts matter so much in bulkheaded systems. The timeout directly determines how many threads you need, which determines how much resource overhead the bulkhead pattern adds. Aggressive timeouts (200-500ms for external calls) keep bulkhead sizes manageable.

Monitor your bulkhead utilization in production. If a pool consistently runs at 80%+ utilization during normal traffic, it's too small. If it never exceeds 20%, it's too large and you're wasting resources. Target 40-60% utilization during peak normal traffic, leaving headroom for spikes.

## Kubernetes-Native Bulkheads

If you're running on Kubernetes, you get several bulkhead mechanisms for free.

Pod resource limits isolate CPU and memory at the container level. One pod can't starve another pod on the same node. Set requests (guaranteed minimum) and limits (maximum allowed) for every pod.
...
Pod Disruption Budgets ensure that maintenance operations (node drains, rolling updates) don't take down too many pods at once. If your checkout service has 10 pods and a PDB of `minAvailable: 8`, Kubernetes will never voluntarily evict more than 2 pods simultaneously.
...
These mechanisms complement application-level bulkheads. Kubernetes handles isolation between services. Application-level bulkheads handle isolation within a service. You need both.
...
Week 2: Add a bulkhead for your most dangerous external dependency — the one that's most likely to go slow or fail. Give it a dedicated thread pool with a size based on the formula above. Add a timeout. Add a fallback.
...
The bulkhead pattern isn't glamorous. It's not a new algorithm or a clever architecture. It's plumbing — the kind of boring infrastructure work that prevents spectacular failures. Ships don't sink because of bulkheads. Your services shouldn't go down because one dependency got slow.
