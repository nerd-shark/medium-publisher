...
changes-from-v5: Expanded "The Chaos Engineering Lifecycle" section with discussion paragraphs in each subsection to reduce reliance on bullet points and provide more context
---
...
The hardest part isn't technical—it's getting leadership to approve intentionally breaking production. From their perspective, you're asking to create the very problems they pay you to prevent. Their job is to minimize risk and maximize uptime, and here you are proposing to inject failures into a working system. The conversation requires reframing chaos engineering from "breaking things" to "preventing much bigger breaks."
...
Once you have organizational buy-in, you need a structured approach to running experiments. Chaos engineering isn't random destruction—it follows a scientific method with five key steps that ensure your experiments are controlled, measurable, and valuable.

### 1. Define Steady State

Steady state is what "normal" looks like for your system, measured in business metrics that users actually experience. Focus on response times, error rates, and throughput—not CPU usage or memory consumption.
...
A good hypothesis is specific, measurable, time-bound, and falsifiable—it forces you to define exactly what should happen when you inject a failure.
...
The failures you inject should mirror what actually happens in production: servers crash, networks partition, and dependencies time out. Start simple, then progress to compound failures.
...
Run the experiment and actively look for ways your hypothesis might be wrong. This scientific approach helps you catch subtle degradations that you'd miss if you were just hoping everything works.
...
Once an experiment passes, automate it to run weekly. This ensures your system remains resilient as code changes and trains your team to expect failures.
...
Start with low-risk experiments in non-production environments before progressing to critical systems. Killing a production database affects every user instantly—terminating a single API server in staging affects no one. Here's where to begin:
...
Chaos engineering is controlled chaos, not reckless destruction. Every experiment needs guardrails to limit the impact if something goes wrong. Here's how to keep experiments safe while still learning valuable lessons:
...
You can't improve what you don't measure. Track two things: the experiments themselves (leading indicators) and the real-world incidents they help prevent (lagging indicators).
...
**The hidden failure**:
- No progressive rollout - WAF rule deployed globally in one go
- No automatic rollback - took 27 minutes to manually rollback when CPU spiked
- "Simulated mode" assumed safe, but still evaluated regex on every request
...
Once you're running regular experiments and finding bugs, it's time to level up. Advanced chaos engineering means expanding scope (GameDays), increasing frequency (continuous chaos), and treating experiments as code (version control, CI/CD).
...
Where does your organization fall on the chaos engineering maturity spectrum? Most teams start at Level 0 (reactive) and progress through increasingly sophisticated practices. The goal isn't to reach Level 4 immediately—it's to move up one level at a time, building confidence and capability.

### Level 0: No Chaos Engineering

You're flying blind. Incidents happen, you react, but you're not proactively testing failure scenarios.
...
You're dipping your toes in. Manual experiments in staging, occasional production tests, but nothing systematic yet.
...
You've established a rhythm. Weekly experiments, automated rollback, production testing with proper controls.
...
Chaos is part of your culture. Daily automated experiments, quarterly GameDays, chaos integrated into CI/CD.
...
Chaos is everywhere, all the time. Continuous low-level chaos, chaos in every deployment, resilience is a first-class concern.
...
1. **GitHub October 2018 Outage** - 43-second network partition caused 24 hours of degraded service
   - Official Post-Mortem: https://github.blog/2018-10-30-oct21-post-incident-analysis/
   - Published: October 30, 2018
   - Source for network partition incident details and recovery timeline

2. **Cloudflare July 2019 Outage** - Regex in WAF rule caused global 30-minute outage
   - Official Post-Mortem: https://blog.cloudflare.com/cloudflare-outage/
   - Published: July 2, 2019
   - Source for regex catastrophe, CPU spike, and rollback timeline

3. **AWS S3 February 2017 Outage** - Typo took down S3 for 4 hours
   - Official Post-Mortem: https://aws.amazon.com/message/41926/
   - Published: March 2, 2017
   - Source for S3 subsystem failure and $150M impact estimate

### Chaos Engineering Maturity Model

4. **"Chaos Engineering Maturity Model"** - Gremlin
   - https://www.gremlin.com/chaos-engineering/maturity-model/
   - Framework for assessing chaos engineering adoption levels
   - Source for Level 0-4 maturity progression

5. **"Principles of Chaos Engineering"** - Chaos Engineering Community
   - https://principlesofchaos.org/
   - Foundational principles and practices
   - Source for steady-state hypothesis and blast radius concepts
...
6. **AWS Fault Injection Simulator (FIS)**
   ...
   - Supports EC2, ECS, EKS, RDS fault injection

7. **Gremlin**
   - https://www.gremlin.com/
   - Commercial chaos engineering platform
   - Comprehensive failure injection across infrastructure and applications

8. **Chaos Toolkit**
   - https://chaostoolkit.org/
   - Open-source chaos engineering toolkit
   - Extensible, declarative experiment definitions

9. **Litmus**
   - https://litmuschaos.io/
   - CNCF chaos engineering framework for Kubernetes
   - Cloud-native chaos experiments

10. **Chaos Mesh**
    - https://chaos-mesh.org/
    - CNCF chaos engineering platform for Kubernetes
    - Supports network, pod, stress, and time chaos

### Books and Additional Reading

11. **"Chaos Engineering: System Resiliency in Practice"** - Casey Rosenthal and Nora Jones
    - O'Reilly Media, 2020
    - Comprehensive guide to chaos engineering principles and practices

12. **"Site Reliability Engineering: How Google Runs Production Systems"** - Google
    - O'Reilly Media, 2016
    - Source for SRE practices and reliability engineering

13. **"Release It! Design and Deploy Production-Ready Software"** - Michael Nygard
    - Pragmatic Bookshelf, 2nd Edition, 2018
...
**Previous Article**: [Part 1: Building Resilient Systems - The Foundation of Reliability](#)

**Next Article**: [Part 3: Observability and Monitoring - You Can't Fix What You Can't See](#)

**Coming Up in This Series**:
- Part 4: Incident Response and Post-Mortems - Learning from Failure
- Part 5: SRE Practices - Error Budgets and SLOs
- Part 6: Distributed Systems Resilience - Handling Partial Failures
- Part 7: Database Resilience - Backup, Recovery, and Failover Strategies
