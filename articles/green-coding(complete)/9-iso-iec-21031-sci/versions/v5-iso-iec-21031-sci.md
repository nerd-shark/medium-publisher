---
title: "Stop Guessing, Start Measuring: The ISO Standard for Software Carbon Intensity"
subtitle: "One equation. Four variables. ISO/IEC 21031 is the first international standard for scoring software carbon — and you can't buy your way to a better number."
...
changelog: "v5 - Voice and authenticity pass. Broke fake contrast pairs, softened absolute claims, varied case study structure, added personal asides, broke triplet pattern in closing. Substance unchanged."
---
# Stop Guessing, Start Measuring: The ISO Standard for Software Carbon Intensity
...
![SCI Formula Breakdown](../images/Graphic1.png)
...
| Country     | Typical Intensity | Primary Source   | Cloud Region               |
| ----------- | ----------------- | ---------------- | -------------------------- |
| Norway      | ~30 gCO₂/kWh     | Hydro            | Azure Norway East          |
| Sweden      | ~45 gCO₂/kWh     | Hydro/Nuclear    | AWS eu-north-1             |
| France      | ~56 gCO₂/kWh     | Nuclear          | AWS eu-west-3              |
| Brazil      | ~80 gCO₂/kWh     | Hydro            | AWS sa-east-1              |
| Canada      | ~120 gCO₂/kWh    | Hydro/Nuclear    | AWS ca-central-1           |
| Germany     | ~350 gCO₂/kWh    | Coal/Renewables  | AWS eu-central-1           |
| US Average  | ~400 gCO₂/kWh    | Mixed            | AWS us-east-1              |
| South Korea | ~420 gCO₂/kWh    | Coal/Gas/Nuclear | AWS ap-northeast-2         |
| Singapore   | ~430 gCO₂/kWh    | Natural Gas      | AWS ap-southeast-1         |
| Japan       | ~470 gCO₂/kWh    | Gas/Coal         | AWS ap-northeast-1         |
| Australia   | ~480 gCO₂/kWh    | Coal/Solar/Wind  | AWS ap-southeast-2         |
| China       | ~550 gCO₂/kWh    | Coal/Renewables  | AWS cn-north-1             |
| Poland      | ~670 gCO₂/kWh    | Coal             | Azure Poland Central       |
| India       | ~680 gCO₂/kWh    | Coal/Solar       | AWS ap-south-1             |

![Carbon Intensity by Region](../images/Graphic2.png)
...
## Choosing R — Where Most Teams Get Stuck

This is where teams stall out, and I'd argue it's the choice that matters most — more than which tool you use to measure E, more than whether you pick location-based or market-based intensity. R must be meaningful to your software. Something a developer can look at and know what to optimize.
...
| Agentic AI      | Per agent invocation | Full cost: LLM + tools + orchestration |
...
E × I dominates (1.9g) and M is small (0.027g). That tells you where to focus: reduce energy consumption or move to a cleaner region. Extending hardware life won't move the needle for this service. I'll be honest — the first time I ran this calculation on one of my own services, I expected M to be the big number. It wasn't even close. E × I was 98% of the score. That reframed where I spent my optimization time.
...
The UBS case study is interesting for a different reason than the others. They applied SCI to two on-premises banking applications — one in Investment Banking, one in Asset Management — and ran into the problem that cloud-native teams never think about: where do you get power data when there's no dashboard?

They ended up working with facilities teams to get actual power measurements and estimate allocation across shared infrastructure. It took cross-team coordination that a team running on AWS would never need. But it proved the methodology works outside the cloud, which matters for any organization with significant on-prem footprint.
...
CSRD asks "what are your total emissions?" SCI can answer something more useful: here's the emissions rate per unit of software output, and here's the trend line showing whether engineering effort is actually reducing it. Compliance gets you the first answer. SCI gets you the second.
...
SCI is a rate — it tells you you're efficient per unit of work. It doesn't tell you anything about your total footprint. A very efficient service running at massive scale still emits a lot of carbon in absolute terms. Keep that distinction in mind when reporting results.

SCI is the best tool available for measuring software carbon intensity. It's not perfect — and I've spent enough time with it to know the rough edges personally. Use it directionally, improve your data quality over time, and don't let perfect be the enemy of useful.
...
![Series Recap — Every Article Was a Lever on SCI](../images/Graphic4.png)
...
**Week 2:** Set up automated SCI tracking. Cloud Carbon Footprint for energy and embodied emissions, Electricity Maps API for carbon intensity. Put it in a dashboard next to your latency and error rate metrics — that's where it belongs.

**Week 3:** Look at the breakdown. Is E the biggest contributor? Then focus on code efficiency. Is I high because you're running in a coal-heavy region? That's a different conversation. The SCI decomposition tells you which lever to pull.

**Week 4:** Set a target. 10% reduction in 6 months is realistic for most teams. Review it in sprint retros the same way you'd review latency regressions.
...
This is the series finale. Thank you for following along through nine articles on sustainable software engineering. The standard exists and the tooling is catching up. Whether any of this matters depends on whether teams actually start measuring — and I think more will, once they see how little effort the first baseline takes.

---

*Daniel Stauffer is an Enterprise Architect specializing in AI systems and platform engineering. He writes about building systems that work well without costing the planet more than they should.*
