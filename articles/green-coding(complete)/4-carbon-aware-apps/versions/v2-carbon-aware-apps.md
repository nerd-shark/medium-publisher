...
changes-from-v1: Moved "When NOT to Use" section to early "Who's This Article For" section as reader off-ramp
---
...
Before we dive deep, let's be clear about whether carbon-aware patterns make sense for your situation.

**These techniques work best if you're working with**:

- **Batch processing workloads** - Data pipelines, ETL jobs, report generation
- **ML model training** - Training jobs that take hours or days
- **Background jobs** - Video encoding, image processing, data backups
- **Multi-region infrastructure** - Already running in multiple cloud regions
- **Deferrable workloads** - Tasks that can wait hours for cleaner grid conditions
- **Large-scale operations** - Workloads where carbon savings justify the complexity

**These techniques may not apply directly if you're dealing with**:

- **User-facing APIs** - Real-time requests where latency matters
- **Time-sensitive processing** - Financial transactions, real-time analytics
- **Single-region deployments** - No ability to shift workloads geographically
- **Small workloads** - The overhead isn't worth it for tiny jobs
- **Strict cost constraints** - Multi-region and data transfer costs add up
- **Latency-critical systems** - Anything where milliseconds matter

That said, if you care about reducing carbon emissions in software, keep reading. Understanding how the grid works and when it's cleanest will help you think differently about infrastructure decisions, even if you can't implement these specific patterns today. You might find opportunities to apply carbon-aware thinking in unexpected places.

Carbon-aware computing adds complexity. It requires thinking about when and where your code runs, not just how it runs. But the environmental impact can be substantial—20-50% carbon reduction for workloads that can tolerate delays or geographic shifts.

Still here? Great! Let's talk about how the grid works.
